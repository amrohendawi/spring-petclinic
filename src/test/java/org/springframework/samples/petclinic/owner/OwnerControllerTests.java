...
class OwnerControllerTests {
    ...
    void setup() {
        ...
        Owner george = george();
        // Add a second pet with the same name "Max" that is new (no id set) to test getPet selection logic
        Pet newMax = new Pet();
        PetType dog = new PetType();
        dog.setName("dog");
        newMax.setType(dog);
        ...
        // Note: newMax is intentionally not assigned an id, so it is considered new
        ...
        // This use of getPet("Max") should return the persisted pet (with id=1), not the new one
    ...
    void testShowOwner() throws Exception {
        mockMvc.perform(get("/owners/{ownerId}", TEST_OWNER_ID))
            ...
            // Added assertion to verify that getPet returns the persisted pet (with id==1) even if there is a new pet with the same name
            .andExpect(result -> {
                Owner owner = (Owner) result.getModelAndView().getModel().get("owner");
                Pet pet = owner.getPet("Max");
                if (pet == null) {
                    throw new AssertionError("Expected pet \"Max\" to be found, but got null.");
                }
                if (pet.getId() == null || pet.getId() != 1) {
                    throw new AssertionError("Expected persisted pet with id 1, but got pet with id: " + pet.getId());
                }
            });
    }
