...
class OwnerControllerTests {
	...
	void setup() {
		...
		// This call exercises the branch where the pet exists
	...
	void testShowOwner() throws Exception {
		mockMvc.perform(get("/owners/{ownerId}", TEST_OWNER_ID))
			...
			.andExpect(result -> {
				// Additional assertions to cover the getPet method conditional branches
				Owner owner = (Owner) result.getModelAndView().getModel().get("owner");
				// Expect getPet to return a valid pet when the name matches exactly
				assertNotNull(owner.getPet("Max"), "Expected to find pet named 'Max'");
				// Expect getPet to return null when the pet name does not exist
				assertNull(owner.getPet("Fido"), "Expected not to find pet named 'Fido'");
			});
	}
