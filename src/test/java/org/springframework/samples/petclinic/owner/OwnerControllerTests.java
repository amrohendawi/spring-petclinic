 ...
 *
 * Modified to add explicit assertions on Owner#getPet to kill a surviving mutation induced by negated conditionals in Owner#getPet.
 *
 * @author Colin But
 * @author Wick Dynex
 */
...
class OwnerControllerTests {
	...
	void testShowOwner() throws Exception {
		mockMvc.perform(get("/owners/{ownerId}", TEST_OWNER_ID))
			...
			.andExpect(mvcResult -> {
				// Additional assertions to explicitly test Owner#getPet behavior
				Owner owner = (Owner) mvcResult.getModelAndView().getModel().get("owner");
				// Assert that getPet returns the pet with name "Max"
				assertNotNull(owner.getPet("Max"), "Expected to find pet with name 'Max'");
				// Assert that getPet returns null when pet name is not present
				assertNull(owner.getPet("Buddy"), "Expected no pet with name 'Buddy'");
			});
	}
