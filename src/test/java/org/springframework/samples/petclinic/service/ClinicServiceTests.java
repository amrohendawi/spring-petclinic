...
class ClinicServiceTests {
	...
	void shouldAddNewVisitForPet() {
		...
		// Re-fetch the owner and pet from the database to ensure persistence
		optionalOwner = this.owners.findById(6);
		assertThat(optionalOwner).isPresent();
		owner6 = optionalOwner.get();
		pet7 = owner6.getPet(7);

		// Verify the persisted visit has been added
		assertThat(pet7.getVisits()).hasSize(found + 1).allSatisfy(v -> {
			assertThat(v.getId()).isNotNull();
			assertThat(v.getDescription()).isEqualTo("test");
